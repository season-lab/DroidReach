...
class FindClass(NativeDroidSimProcedure):
    def run(self, env, name):
        ...
        name_str = self.state.solver.eval(self.state.memory.load(name, name_strlen.ret_expr), cast_to=bytes).decode("ASCII")
        ...
        NativeDroidSimProcedure.class_map[jclass.ptr] = name_str
        return return_value
...
class RegisterNatives(NativeDroidSimProcedure):
    def run(self, env, clazz, methods, nMethods):
        ...
        class_str = "???"
        if not self.state.solver.symbolic(clazz):
            v = self.state.solver.eval(clazz)
            if v in NativeDroidSimProcedure.class_map:
                class_str = NativeDroidSimProcedure.class_map[v]
        ...
        for i in range(method_num):
            ...
            dynamic_map['%s:%s:%s' % (class_str, name.decode("ASCII"), signature.decode("ASCII"))] = fn_ptr
...
class JNINativeInterface(ExternObject):
    ...
    def __init__(self, project, analysis_center):
        super(JNINativeInterface, self).__init__(project.loader,
            map_size=len(self.JNINativeInterface_index_to_name) * 8)
        self._provides = 'JNIEnv'
        self._project = project
        self._fptr_size = self._project.arch.bits // 8
        self._project.loader.dynamic_load(self)
        self._analysis_center = analysis_center
        self._construct()
    ...
    def _construct(self):
        ...
        self.memory.pack_word(self._JNIEnv - self.min_addr, self._JNINativeInterface)

        # direct calls hook
        # for addr in self._project.loader.main_object.symbols_by_addr:
            # symb = self._project.loader.main_object.symbols_by_addr[addr]
        for symb in self._project.loader.main_object.symbols:
            addr = symb.rebased_addr
            if symb.name in self.JNINativeInterface_sig:
                ...
                if jni_native_interface_func_name in self.JNINativeInterface_name_to_simproc:
                    ...
                    self._project.hook(addr, proc, replace=True)
                else:
                    self._project.hook(addr, angr.SIM_PROCEDURES['stubs']['ReturnUnconstrained'](), replace=True)
        ...
        for index, name in self.JNINativeInterface_index_to_name.items():
            ...
            if name.startswith('reserved'):
                ...
                self._project.hook(addr, angr.SIM_PROCEDURES['stubs']['PathTerminator'](), replace=True)
            else:
                addr = self.allocate(self._fptr_size)
                ...
                # otherwise hook with ReturnUnconstrained
                    ...
                    self._project.hook(addr, angr.SIM_PROCEDURES['stubs']['ReturnUnconstrained'](), replace=True)
            self.memory.pack_word(self._JNINativeInterface - self.min_addr + index * self._fptr_size, addr)
