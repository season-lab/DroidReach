...
from ..jni_type.jni_native_interface import JNINativeInterface
...
class JNIInvokeInterface(ExternObject):
    ...
    def __init__(self, project, analysis_center):
        super(JNIInvokeInterface, self).__init__(project.loader,
            map_size=len(self.jni_invoke_interface_index_to_name) * 8)
    ...
    def _construct(self):
        ...
        self.memory.pack_word(self._JavaVM - self.min_addr, self._JNIInvokeInterface)
                ...
                if name in self.jni_invoke_interface_index_to_simproc:
                    self._project.hook(addr,
                                       self.jni_invoke_interface_index_to_simproc[name](
                                           JNIEnv_ptr=self._JNIEnv), replace=True)
                # otherwise hook with ReturnUnconstrained
                else:
                    self._project.hook(addr, angr.SIM_PROCEDURES['stubs']['ReturnUnconstrained'],
                                       kwargs={'resolves': name}, replace=True)

            self.memory.pack_word(self._JNIInvokeInterface - self.min_addr + index * self._fptr_size, addr)
